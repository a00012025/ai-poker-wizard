...
`app.gtowizard.com`，Extension 會把頁面正在使用的 GTO Wizard session bundle
（access token、refresh token、GWCLIENTID 與期限）同步到後端。後端會沿用瀏覽器的
access token 到它實際過期，只有過期後才自行 refresh，避免建立多餘 session 導致
網頁被登出。v2.1 起，Extension popup 提供「♠ 同步手牌到 DB」按鈕：一鍵把新上傳的
Analyze 手牌攝取進 ledger，popup 顯示進度，完成後 Telegram 也會收到結果。

## 安裝

1. 到最新 GitHub Release 下載 `ai-poker-wizard-gtow-sync-v2.3.0.zip`。
2. 解壓縮下載檔案。
...
會先把當前 session bundle 同步上去，再排入攝取佇列；bot 幾秒內接手，優先沿用
瀏覽器 access token 跑 incremental ingest（對數不符時自動全量補齊）。若 access
token 已實際過期，後端才會用同一組持久化 signing keypair refresh 一次。進度顯示在 popup，
結果同時發到 Telegram。
...
- Extension 只在 `app.gtowizard.com` 觀察頁面自己送出的 GTOW API authorization、
  `GWCLIENTID`，以及 `localStorage.user_refresh`。
- Extension 不會顯示、記錄，或將原始 GTOW token 寫入 Chrome storage；session bundle
  只保留在記憶體並透過已配對的 HTTPS device-auth 通道同步。只有使用者
...
- 同步服務以 access JWT 的 `iat` 防止舊裝置覆蓋較新的瀏覽器 session。
- GTOW 回傳 401 但 access JWT 尚未過期時，服務不會自行 refresh；會保留原始錯誤，
  等 Extension 捕捉瀏覽器的新 session。
