...
description: Refresh the local GTO Wizard token when the bot reports token expiry or a user asks to log back into GTO Wizard. Use for "更新 GTO token", "refresh gto token", "GTO Wizard 過期", or manual GTO login/token refresh tasks.
user_invocable: true
---
...
Do NOT replace `.tokens.json` with a new file path/inode.
