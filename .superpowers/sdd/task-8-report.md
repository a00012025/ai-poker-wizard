...
- Added `_apply_live_resend(...)` to briefly load session metadata, run synchronous parse/solver grading via `asyncio.to_thread` before acquiring the write connection, then edit the original live-session report in place when possible. If editing the original message fails, the fallback report message is persisted via `set_session_message`.
...
- `apply_live_resend_overwrites_session_and_edits_original_message` — fake integration coverage for to-thread pre-parse, no write connection held during parsing, apply flow, page render, original message edit, and confirmation.
...
- Parse/solver work for resend now runs via `asyncio.to_thread`; the DB transaction covers only the atomic write section.
