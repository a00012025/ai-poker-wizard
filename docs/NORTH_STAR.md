...
> **性質**: 單一選手（N=1）的個人訓練系統
> **讀者**: 專案擁有者（選手本人），以及未來接手規劃/實作的任何 AI 模型與協作者
...
1. 開始任何新 feature 的規劃前，先讀完本文件（至少 §2、§3、§7、§8、§13、§14）。
2. 你的提案必須通過 §13 的對齊檢查清單。過不了就修改提案，不要修改本文件來遷就提案。
3. 本文件刻意停留在 high-level（目的、邊界、不變量、驗收訊號），把實作自由度留給你 — 但**不變量（§7）沒有商量空間**。
4. 修改本文件需要專案擁有者明確同意，且任何修改要在 §16 版本紀錄留痕。

---

## 0. 使用情境與設計前提（2026-07 snapshot）

任何訓練系統都是為特定使用情境設計的。本節是設計前提的事實基準，隨時間更新；它描述「系統為什麼長這樣」，不是使用者檔案。

- **使命**: 以 AI + solver 工具訓練出一位世界頂尖的德州撲克錦標賽（MTT）牌手。系統為單一選手深度客製（N=1）。
- **主戰場**: GGPoker 線上 MTT，買入 **$1–$10**，**PKO 與一般 MTT 都打**；同時有**線下 MTT**（比重漸增）。
- **訓練時間預算**: 兼職訓練型態 — 打牌時數為主，study 時間每週個位數小時。課表與功能設計必須適配這個預算（§5.10），塞爆課表是依從性殺手。
- **既有工作流**（系統必須銜接，不是取代）:
...
- **週課表生成**：按 `期望收益 = EV loss 金額 × 出現頻率 × 可學性` 挑 1–2 個焦點 family，把中圈的五個環節（研究→壓縮→drill→重測）排進**真實可用的時間預算**（設計前提：每週個位數小時，§0）。排不完自動降載 — 課表塞爆是依從性殺手。
...
**目的**：選手常態投入的課程/書/影片學習目前在系統外流失。終態：一切內容學習匯入同一條複利管線。
...
本節是定稿模型留給後續接手者的 meta 層叮嚀。§7 是產品不變量；本節是**開發行為**的原則 — 每一條都對應本專案已付過學費的教訓，或單人 AI 專案特有的重力。

### 認知紀律

1. **先重現，再修改。** 對任何「壞掉」的判定，先用原始資料（HH 檔 / 截圖）重跑當前管線再下結論。本 repo 歷史上大量「bug 報告」其實是過期的中間狀態（stale expected_json、未鎖定的快取鍵、舊版解析結果）。不要信任儲存的中間態，一律從源頭重推。
2. **大聲失敗，不要安靜降級。** Silent fallback 是本專案歷史上最貴的 bug 類型（ICM→chipEV 靜默替代、解析錯誤被吞成「無 solver 數據」）。所有降級必須可觀測、進誠實層、在 log 與使用者面可追。
3. **統計主張旁邊沒有 n 的，當它不存在。** Family × 深度 × 賽段的切分讓樣本碎得非常快；一切聚合設樣本下限、小樣本用收縮估計、任何呈現都帶 n。這條同時約束 leak 榜、pool 挖礦與進步敘述。
4. **尊重變異數。** 兩週的指標移動不是趨勢，技能趨勢是月尺度；撲克結果的噪音遠超人類直覺。沒有變異數框架（信賴區間、樣本量、期間）的進步/退步結論禁止輸出 — 錯誤的進步敘事對選手心態的傷害比沒有資訊更大。

### 產品重力（會自然發生、需要主動抵抗的力）

5. **教練會退化成解說員。** LLM 擅長生成解釋，所以系統會自然長出越來越多「給人讀的東西」。每個新輸出面自問：它逼選手先作答了嗎（不變量 1）？多一份沒人讀的報告是負分。
6. **工程會流向保真度而不是迴圈。** 精度有明確指標、迴圈缺口沒有，所以最佳化會自然滑向「+2% 精度」而不是補全迴圈。做選擇時永遠先補迴圈缺口（§10 排序原則的日常版）。
7. **新輸出面會繞過驗證器。** 歷史上多次幻覺 bug 的模式相同：新功能沒接上既有的事實驗證管線。規則：任何新的使用者可見輸出，上線前必須路由過 fact-verification 層，無例外。
8. **憲法會誘發大教堂。** 本文件描述方向，不是首版規格。每個 phase 只造「能讓該層迴圈轉起來的最小版本」；§6 的 schema 是語義契約，不是平台邀請函。懷疑時選更無聊的技術、更小的版本。

### 協作姿態

9. **本文件授權你反對。** 專案擁有者的臨時想法與 §7/§8/§13 衝突時，引用條文提出反對是被期待的行為，不是越權。單人專案沒有 PM — 這份文件就是 PM；順從地實作一個違反不變量的請求，是最壞的服務。
10. **開發對話中不要客串教練。** 策略問題一律走 grounded 管線回答；開發情境下的模型不憑記憶給撲克策略建議。系統的核心承諾是「每個策略主張都有 solver 事實背書」，開發者自己先破戒，產品就沒有立場。
11. **對事實直接，對人溫和。** 教學輸出基於 solver 事實，即使結論讓人失望也不迎合；但語氣永不羞辱，推播可跳過、可延後 — 依從靠尊重不靠壓力。
12. **完成 = 驗收訊號被觀測，不是測試通過。** Regression test 是地板（每個 bug fix 必附，文化延續）；「完成」的定義是該功能的驗收訊號在真實資料上、跨時間地被觀測到。Prompt 改動也是行為改動，同樣走快照對照。
13. **維護連續性。** 大改 merge 後檢查 §0/§12 是否過時；重大決策與對本文件的偏離寫進 docs/handoffs；文件漂移本身是專案風險。你的下一個接手者跟你一樣需要乾淨的上下文。
...
- 不變量（§7）與非目標（§8）的任何修改：需專案擁有者明確同意 + 在 §16 留痕。
- 每完成一個 Phase（§10），回頭核對驗收訊號並更新狀態。

## 16. 版本紀錄

- **1.0（2026-07-07）**：初版（協作定稿模型：Claude Fable 5）。內容：定位（一人職業戰隊、GTO Wizard 為底座的教練層）、北極星指標與歸因機制、四層迴圈、11 個子系統、12 條不變量、四階段路線圖、給未來開發者的備忘錄（§14）。定稿依據：repo 全量探索、職業 MTT 訓練方法研究、2026 工具市場調查與多輪討論。
